...
Concrete examples of what a good **section outline** (pre-write gate) looks like for each of the two docs this skill produces. Use these as calibration, not as literal templates — the sections must match the actual task.

**Note:** CONTEXT.md is *input* to this skill, not output. If CONTEXT.md doesn't exist yet, run `create-planning-docs` first. The examples below cover PLAN.md and REFERENCES/RESEARCH.md only.
...
- **Context reference** — cites CONTEXT.md by path (e.g., `../CONTEXT.md`) as the source of what/why; does not re-state scope, success criteria, or non-goals
...
- **Context reference** — cites CONTEXT.md by path as the source of what/why; does not re-state scope, success criteria, or non-goals
...
Use these to evaluate the success criteria *already in CONTEXT.md* — if CONTEXT.md's criteria are vague, flag it in the clarifying round (Step 2) before writing PLAN, because PLAN's exit gates must verify them.
...
When CONTEXT.md or the conversation leaves something ambiguous, mark it explicitly in PLAN rather than guessing:
...
## Self-review checklist (run internally before presenting the PLAN outline)

This checks that CONTEXT.md is fit to plan against, and that PLAN's outline will be derivable from it. Before showing the PLAN outline to the user, verify:

- [ ] CONTEXT.md exists at the confirmed path and was read in full
- [ ] CONTEXT.md states scope (in and out) — if vague, flag in clarifying round
- [ ] CONTEXT.md has success criteria (measurable if possible) — if vague, flag in clarifying round so PLAN exit gates can verify something concrete
- [ ] WHAT/HOW separation holds in CONTEXT.md — no tech stack or implementation leaked in; if it has, PLAN should ignore the leaked how, not inherit it
- [ ] Cited artifact paths in CONTEXT.md exist on disk
- [ ] REFERENCES skip is decidable (was there new research in the conversation beyond what CONTEXT.md already captured, or not?)
- [ ] Any ambiguity that would make the PLAN outline wrong is either resolved via clarifying round or surfaced back to CONTEXT.md as an Open Question
...
This is the 3rd gate — run it after the final doc is written but before Step 4. It checks the docs are *mutually consistent*, not just that each is internally well-formed. CONTEXT.md is the input doc; PLAN.md and REFERENCES/RESEARCH.md are the outputs.
...
> PLAN.md is written to `b0ttsagent/planning/FirstFix/PLAN.md` (7.2 KB).
> Please review the written content and let me know if it's approved or needs changes before I move on to REFERENCES/RESEARCH.md.

**Batch mode** (when user pre-authorized "write all, review at end"):
> PLAN.md written (7.2 KB). Moving to REFERENCES/RESEARCH.md outline.
> [presents REFERENCES outline immediately]
...
> Living-doc note: if a constraint is violated or an assumption turns wrong during execution, update CONTEXT.md before continuing; if PLAN phases drift from the actual implementation, update PLAN.md.
