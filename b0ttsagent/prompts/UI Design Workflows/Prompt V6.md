Ask me any questions, and review with me before starting. Make sure you spawn general sub agents as the leads for each phase/wave. b0ttsresearcher for research agent, explorer agents for misc non lead. Everything should be already setup and working/ready, so we are ready to start, we just gotta follow the workflow. 

Utilize general, b0tts lead researcher, and b0tts researcher sub agents. Only lead researcher sub agent can spawn its own sub agents, so dont give sub agent tasks to b0tt researcher or general agent sub agent.
