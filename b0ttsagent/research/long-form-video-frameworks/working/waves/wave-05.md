...
**Wave goal:** verify 4 more candidates with the same strict protocol. Run-wide: 11 PASS banked; target ≥12 PASS (prefer 15).
...
1. Drew Gooden — https://www.youtube.com/@drewgooden — slug `drew-gooden`
2. Wendover Productions (Sam Denby) — https://www.youtube.com/@wendoverproductions — slug `wendover-productions`
3. Solar Sands — https://www.youtube.com/@solarsands — slug `solar-sands`
4. Dan Mace — https://www.youtube.com/@DanMace — slug `dan-mace`

Each worker gets their candidate's full row from `working/candidates.md`.

**WORKER TYPE: `b0tts-general-agent`** (shell access for yt-dlp — b0tts-researcher lacks shell; do not use it for verification). Lead spawns all workers in ONE message, parallel.
...
**Lead output:** `working/waves/report-05.md` — per-worker status, anomalies, next actions. Lead final message ≤500 words.
